...
struct task_struct
{
	...
	struct task_struct *prev, *next;	/* Used to link the task struct in the 
										   ...
										   runqueue */
/* ---------------- Do NOT Touch END-------------- */
	/* SJF Accounting Fields */
	unsigned long long burst_start_time; /* When the task last got the CPU */
	unsigned long long last_burst;       /* The actual duration of the last burst */
	unsigned long long exp_burst;        /* Expected future burst */
	unsigned long long last_active_time; /* When it last executed or entered the RQ */
};
