...
static void runcpu()
{
	...
	}while(rq->nr_running > 1 || (rq->nr_running == 1 && !init->thread_info->kill));
}
...
static int taskEnd()
{
	/* PROTECTION: The Init task must never be freed by killtask() 
	while the runcpu() loop is still active. */
	if (current == init)
	    return 0;
...
static void killtask(struct task_struct **p)
{
	...
	/* FIX to the double-free: Remove from the parent's child list (clist) */
    /* If we don't do this, the parent's spawnChildren() or cleanuptask() 
       will follow a pointer to freed memory. */
    if (j->thread_info->clist.next != NULL && j->thread_info->clist.next != &j->thread_info->clist) {
        list_del(&j->thread_info->clist);
    }
